...
namespace SevenDTDMono.Utils
{
    public static class CGUILayout
    {
        ...
        public static bool Button(string label, Color active, Color inactive, params GUILayoutOption[] options)
        {
        ...
        public static bool Button(string label, Color normal, Color hover, Color active, params GUILayoutOption[] options)
        {
            ...
            if (currentEvent.type == EventType.Repaint)
            {
                ...
                bool isMouseOver = buttonRect.Contains(currentEvent.mousePosition);

                textColor = isMouseOver ? (currentEvent.button == 0 ? active : hover) : normal;

                // Set the button text color based on the mouse state
            ...
            buttonStyle.active.textColor = active;
            buttonStyle.onActive.textColor = active;

            bool isClicked = GUI.Button(buttonRect, label, buttonStyle);
            return isClicked;
        }

        public static bool Button1(string label, Color normal, Color hover, Color active, params GUILayoutOption[] options)
        {
            GUIStyle buttonStyle = new GUIStyle(GUI.skin.button);
            buttonStyle.normal.textColor = normal;
            buttonStyle.onNormal.textColor = normal;
